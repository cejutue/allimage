...
struct GifWriter
{
	...
	void(*fputc)(int c, void* f);
	void(*fputs)(const char* c, void* f);
	void (*fwrite)(const void*, int n, int count, void* f);
	void (*close)(void* f);
};
void file_fputs(const char* c, void* f)
{
	fputs(c, (FILE*)f);
}
void file_fputc(int c, void* f)
{
	fputc(c, (FILE*)f);
}
void file_fwrite(const void* buf, int n, int count, void* f)
{
	fwrite(buf, n, count, (FILE*)f);
}
...
void FileWriter(GifWriter* writer,const char* filename)
{
	writer->f = fopen(filename, "wb");
	writer->fputs = &file_fputs;
	writer->fputc = &file_fputc;
	writer->fwrite = &file_fwrite;
	writer->close = &file_close;
}
...
{
	writer->fputc((int)stat.chunkIndex, writer->f);
	writer->fwrite(stat.chunk, 1, stat.chunkIndex, writer->f);
...
{
	writer->fputc(0, writer->f);  // first color: transparency
	writer->fputc(0, writer->f);
	writer->fputc(0, writer->f);
    ...
    for(int ii=1; ii<(1 << pPal->bitDepth); ++ii)
    {
		...
		writer->fputc((int)r, writer->f);
		writer->fputc((int)g, writer->f);
		writer->fputc((int)b, writer->f);
    }
}
...
void GifWriteLzwImage(GifWriter* writer , uint8_t* image, uint32_t left, uint32_t top,  uint32_t width, uint32_t height, uint32_t delay, GifPalette* pPal)
{
	...
	writer->fputc(0x21, writer->f);
	writer->fputc(0xf9, writer->f);
	writer->fputc(0x04, writer->f);
	writer->fputc(0x09, writer->f); // leave prev frame in place, this frame has transparency
	writer->fputc(delay & 0xff, writer->f);
	writer->fputc((delay >> 8) & 0xff, writer->f);
	writer->fputc(kGifTransIndex, writer->f); // transparent color index
	writer->fputc(0, writer->f);

	writer->fputc(0x2c, writer->f); // image descriptor block

	writer->fputc(left & 0xff, writer->f);           // corner of image in canvas space
	writer->fputc((left >> 8) & 0xff, writer->f);
	writer->fputc(top & 0xff, writer->f);
	writer->fputc((top >> 8) & 0xff, writer->f);

	writer->fputc(width & 0xff, writer->f);          // width and height of image
	writer->fputc((width >> 8) & 0xff, writer->f);
	writer->fputc(height & 0xff, writer->f);
	writer->fputc((height >> 8) & 0xff, writer->f);

    //fputc(0, f); // no local color table, no transparency
    //fputc(0x80, f); // no local color table, but transparency

	writer->fputc(0x80 + pPal->bitDepth-1, writer->f); // local color table present, 2 ^ bitDepth entries
    GifWritePalette(pPal, writer);

    const int minCodeSize = pPal->bitDepth;
    const uint32_t clearCode = 1 << pPal->bitDepth;

	writer->fputc(minCodeSize, writer->f); // min code size 8 bits
    ...
    for(uint32_t yy=0; yy<height; ++yy)
    {
        for(uint32_t xx=0; xx<width; ++xx)
        {
            ...
            else
            {
                ...
                GifWriteCode(writer, stat, (uint32_t)curCode, codeSize);
    ...
    GifWriteCode(writer, stat, clearCode + 1, (uint32_t)minCodeSize + 1);

    // write out the last partial chunk
    while( stat.bitIndex ) GifWriteBit(stat, 0);
    if( stat.chunkIndex ) GifWriteChunk(writer, stat);

	writer->fputc(0, writer->f); // image block terminator
...
bool GifBegin( GifWriter* writer, uint32_t width, uint32_t height, uint32_t delay, int32_t bitDepth = 8, bool dither = false )
{
	...
	writer->fputs("GIF89a", writer->f);

    // screen descriptor
	writer->fputc(width & 0xff, writer->f);
	writer->fputc((width >> 8) & 0xff, writer->f);
	writer->fputc(height & 0xff, writer->f);
	writer->fputc((height >> 8) & 0xff, writer->f);

	writer->fputc(0xf0, writer->f);  // there is an unsorted global color table of 2 entries
	writer->fputc(0, writer->f);     // background color
	writer->fputc(0, writer->f);     // pixels are square (we need to specify this because it's 1989)
    ...
    {
        // animation header
		writer->fputc(0x21, writer->f); // extension
		writer->fputc(0xff, writer->f); // application specific
		writer->fputc(11, writer->f); // length 11
		writer->fputs("NETSCAPE2.0", writer->f); // yes, really
		writer->fputc(3, writer->f); // 3 bytes of NETSCAPE2.0 data

		writer->fputc(1, writer->f); // JUST BECAUSE
		writer->fputc(0, writer->f); // loop infinitely (byte 0)
		writer->fputc(0, writer->f); // loop infinitely (byte 1)

		writer->fputc(0, writer->f); // block terminator
    }
...
bool GifEnd( GifWriter* writer )
{
	...
	writer->fputc(0x3b, writer->f); // end of file
	writer->close(writer->f);
